...
NIUQCL_D = LOL_BMP_DIR + "niuqcl_d.bmp"     #扭曲森林（被选择）
NIUQCL_U = LOL_BMP_DIR + "niuqcl_u.bmp"     #扭曲森林（未选择）
...
def server_choise(serverid):
    ...
    try:
        ...
        if serverid == 2:
            click_pic_in_window(hlogin,NIUQCL_U,10,errflg=False)
...
def load_game():
    try:
        ...
        # 建房
        click_pic_in_window(hpvp,ZIDYYX,1,errflg=False)

        # 创建游戏
        click_pic_in_window(hpvp,CHUANGJYX,1,errflg=False)

        # 选择地图和输入密码
        click_pic_in_window(hpvp,TUZHSXG,1,errflg=False)
        click_pic_in_window(hpvp,FANGJMM,1,errflg=False)
